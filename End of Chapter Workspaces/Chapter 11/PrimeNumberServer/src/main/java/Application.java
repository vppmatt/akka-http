...
public class Application {
    ...
    private Route newUpdateRequestV3(Integer requestId) {
        ...
        Source<ByteString, NotUsed> source = Source.single(requestId)
                .via(partialGraph);
    ...
    public Route createRoute() {
        ...
        return get( () ->
                        concat(
                                ...
                                path(segment("result").slash(integerSegment()), (Integer requestId) -> {
                                    CompletionStage<Boolean> result = AskPattern.ask(actorSystem, me ->
                                            new PrimeDatabaseActor.
                                                    DoesRequestExistCommand(me, requestId),
                                    Duration.ofSeconds(5), actorSystem.scheduler());
                                    try {
                                        if (result.toCompletableFuture().get() == true) {
                                            return reject();
                                        }
                                        else {
                                ...
                                path(segment("result").slash(integerSegment()), requestId -> {
