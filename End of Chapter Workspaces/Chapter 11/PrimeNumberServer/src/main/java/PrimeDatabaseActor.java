...
public class PrimeDatabaseActor extends AbstractBehavior<PrimeDatabaseActor.Command> {
    ...
    public static class DoesRequestExistCommand implements  Command {
        private ActorRef<Boolean> sender;
        private Integer requestId;

        public DoesRequestExistCommand(ActorRef<Boolean> sender, Integer requestId) {
            this.sender = sender;
            this.requestId = requestId;
        }

        public ActorRef<Boolean> getSender() {
            return sender;
        }
    ...
    public Receive<Command> createReceive() {
        return newReceiveBuilder()
                ...
                .onMessage(GetResultCommand.class, message -> {
                    BigInteger result = database.get(message.requestId);
                    if (result == null) result = new BigInteger("-1");
                    message.getSender().tell(result);
                    return Behaviors.same();
                })
                .onMessage(DoesRequestExistCommand.class, message -> {
                    message.getSender().tell(database.containsKey(message.requestId));
                    return Behaviors.same();
                })
                .build();
    }
