...
public class HighLevelServer {
    ...
    private Route createRoute() {
        ...
        Unmarshaller<HttpEntity, Customer> customerUnmarshaller = Jackson.unmarshaller(Customer.class);
        Unmarshaller<ByteString, Customer> customerByteStringUnmarshaller = Jackson.byteStringUnmarshaller(Customer.class);
        ...
        Route customerRoute = path("customer", () ->
                concat(
                        ...
                        authenticatedPost( () -> entityAsSourceOf(customerByteStringUnmarshaller,
                                EntityStreamingSupport.json(),
                                source -> {

                                    Sink<String, CompletionStage<String>> sink = Sink.head();

                                    Flow<Customer, String, NotUsed> flow = Flow.of(Customer.class).map( c -> {
                                        actorSystem.tell(new CustomerBehavior.UpdateCustomerCommand(c));
                                        String result = new ObjectMapper().writeValueAsString(c);
                                        return result;
                                    });

                                    CompletionStage<String> graph =
                                      source.viaMat(flow, Keep.right())
                                        .toMat(sink, Keep.right()).run(actorSystem);

                                    return onComplete(graph, s -> complete(s.get()));

                        }) ),
                        //alternative syntax
                        authenticatedPost( () -> entity(customerUnmarshaller, customer -> {
                            Source<ByteString, NotUsed> source = Source.single(1).map( x -> customer)
                                    .via(Flow.of(Customer.class).map( c -> {
                                        actorSystem.tell(new CustomerBehavior.UpdateCustomerCommand(c));
                                        String result = new ObjectMapper().writeValueAsString(c);
