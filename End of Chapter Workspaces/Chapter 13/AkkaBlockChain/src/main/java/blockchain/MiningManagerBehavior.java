...
public class MiningManagerBehavior extends AbstractBehavior<MiningManagerBehavior.Command> {
    ...
    private MiningManagerBehavior(ActorContext<Command> context, StashBuffer<Command> stashBuffer) {
        super(context);
        ...
        return Behaviors.withStash(10,
                stash -> {
                    return Behaviors.setup( context -> {
                        return new MiningManagerBehavior(context, stash);
                    } );
                });
    }
    ...
    private void startNextWorker() {
        if (currentlyMining) {
            ...
            Behavior<MiningWorkerBehavior.Command> workerBehavior =
                    Behaviors.supervise(MiningWorkerBehavior.create()).onFailure(SupervisorStrategy.resume());

            ActorRef<MiningWorkerBehavior.Command> worker = getContext().spawn(workerBehavior, "worker" + currentNonce);
            getContext().watch(worker);
            worker.tell(new MiningWorkerBehavior.Command(block, currentNonce * 1000, difficulty, getContext().getSelf()));
            currentNonce++;
        }
    }
